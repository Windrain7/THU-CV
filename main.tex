...
\setlist[itemize]{
    nosep,                     % 取消 itemize 的默认间距
    before=\vspace{0pt},       % 取消 itemize 和之前段落之间的空白
    after=\vspace{0pt},        % 取消 itemize 和后续段落之间的空白
    leftmargin=*               % 取消 itemize 的左边距
}
\setlist[enumerate]{leftmargin=*}	        % 取消 enumerate 的左边距
\renewcommand{\arraystretch}{1.15}           % 设置表格行间距
\linespread{1.15}                            % 设置正文行间距
...
% 自定义颜色（参考 https://vi.tsinghua.edu.cn/gk/xxbz/scgf.htm）
\definecolor{primary_color}{RGB}{102, 8, 116}    
\definecolor{secondary_color}{RGB}{102, 8, 116}
...
\begin{document}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north, inner sep=0pt](header) at (current page.north){
            \includegraphics[width=\paperwidth]{images/header-purple.png}
        };
        \node[anchor=west](school_logo) at (header.west){
            \hspace{0.5cm}
            \includegraphics[width=0.2\textwidth]{images/logo-banner-white.png}
        };
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south, inner sep=0pt](footer) at (current page.south){
            \includegraphics[width=\paperwidth]{images/footer-purple.png}
        };
        % 联系方式
